...
def main():
    ...
    parser.add_argument(
        ...
        default="outputs/img2img-cad-strength-0-2"
    )
    ...
    import pdb
    data = []
    with open("/path-to-LITE/circuit-diagrams/train.csv") as f:
        f_csv = csv.reader(f)
    ...
    label_map = {}
    with open('/path-to-LITE/circuit-diagrams/label_map.csv') as f:
        f_csv = csv.reader(f)
        for i, row in enumerate(f_csv):
            if i == 0:
                continue
    ...
    for i, row in enumerate(data):
        number = row[0]
        folder_save_path = os.path.join(sample_path, number)
        if not os.path.exists(folder_save_path):
            os.mkdir(folder_save_path)

        img_path = os.path.join(data_path, row[1])
